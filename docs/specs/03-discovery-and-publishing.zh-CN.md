...
- `expires_at`
  Metadata 过期时间。
- `roots`
  厂商根记录（`VendorRootRecord[]`）。
- `qr_signing_certs`
  当前有效或历史 QR 签名证书（`QrSigningCertificate[]`）。
- `revocations`
  根、证书或签名密钥的吊销记录（`RevocationRecord[]`）。
- `mirrors`
  可选的备用分发 URL（`MirrorRecord[]`）。
- `auth`
  信任包本身的签名 metadata（`TrustMetadataAuth`）。

所有 `issued_at`、`expires_at`、`valid_from`、`valid_to` 和 `revoked_at` 值都必须是 RFC 3339 date-time 字符串。生产方应该输出带 `Z` 后缀的 UTC 字符串。诊断 JSON 形式必须把二进制字段包装为 `{ "encoding": "base64url", "value": "<unpadded-base64url>" }`；CBOR 形式必须把同一字段编码为 byte string。

Mirror 是建议性的分发 hint。验证器可以从 mirror 拉取 metadata，但必须像验证主 URL 或本地文件一样验证拉取到的信任包。Mirror URL、DNS 名称、HTTPS 证书或 GitHub 仓库本身不得成为信任锚。
...
被签名的 metadata body 是去掉 `auth` 字段后的 `WtpTrustMetadata`。这样可以避免循环签名，同时让 `roots`、`qr_signing_certs`、`revocations` 和 `mirrors` 都被 root 签名覆盖。
...
当 `revoked_at` 缺失、为空，或验证器时间大于等于 `revoked_at` 时，吊销记录生效。`revoked_at` 必须使用上文定义的 RFC 3339 date-time 格式。生产方应该省略 `revoked_at` 表示立即吊销，而不是输出空字符串。
...
Root 吊销必须提供 `root_fingerprint`。Signing key 或 certificate 吊销必须提供 `key_id`；当同一个 key identifier 可能出现在多个 root 下时，应该同时提供 `root_fingerprint`。
